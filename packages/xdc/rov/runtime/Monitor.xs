...
function print(str)
{
    ...
    /* consecutive print calls (without new lines) append to one another */
    consoleLines[consoleLines.length - 1] += lines.shift();

    /* any additional lines are separate console lines */
    for (var i = 0; i < lines.length; i++) {
        consoleLines.push(lines[i]);

    }
    /* manage line buffer to never exceed the max (by much) */
    if (consoleLines.length > maxLines) {
        var rc = consoleLines.length - maxLines;
        consoleLines.splice(0, rc);
        firstLine += rc;
    }
}
 ...
 *        structure defined; after all, without state to read, there's
 *        nothing to initialize in obj.
 */
...
/*
 *  ======== viewInitConsole ========
 *  args[0] - string: filter (only show lines matching this 'filter')
 *  args[1] - bool  : regexp (is filter a regexp or just a char sequence)
 *  args[2] - number: maxLines (max # of lines to keep in memory)
 */
function viewInitConsole(view, args)
{
    ...
    /* parse args for filter, maxLines, and regexp values */
    var tokens = args.split(/\s*,\s*/);
    var max = Math.abs(Math.ceil(Number(tokens[3])));
    var nargs = {
    ...
    if (maxLines < max) {
        maxLines = max; /* maxLines can only increase */
    }

    /* create a new view element for each line */
    var elements = new Array();

    for (var i = 0; i < consoleLines.length; i++) {
        var line = consoleLines[i];

        /* apply specified console filter */
        if (nargs.regexp) {
            if (line.match(RegExp(nargs.filter))) {
                /* the regexp doesn't match any part of line */
                continue;
            }
        }
        else {
            if (line.indexOf(nargs.filter) == -1) {
                /* line doesn't contain the specified string */
                continue;
            }
        }

        /* line isn't filtered out, so add it to view */
        var elem = Program.newViewStruct('xdc.rov.runtime.Monitor',
                                         'Console');
        elem.lineId = i + firstLine;
        elem.entry = line;
        elements.push(elem);
    }
...
function viewInitSymbols(view, args)
{
    ...
    var val = _parseValue(tokens[0]);
    if (val == null) {
        var msg = "The specified argument '" + tokens[0]
...
function viewInitVariable(view, obj, args)
{
    ...
    if (args == null) {
        return; /* defaults must never trigger an error */
    }

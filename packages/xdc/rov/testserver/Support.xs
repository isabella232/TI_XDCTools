...
function startModel(execPath, urlArgs, logger, progress)
{
    ...
    var dummyCB = xdc.module("xdc.rov.testserver.DummyCallBack").create();
    ...
    if (ip.indexOf("DSLite") == 0) {
        ...
        try {
            progress("starting DSMemoryReader("
                          + dslitePort + ", " + coreId + ", ...) ...");
            model.reader = new Packages.xdc.rov.testserver.DSMemoryReader(
                dslitePort, coreId, model.elf, 0);
        }
        catch (x) {
        ...
        debug.println("can't use port: " + ip);
    }
